...
export const api = {
  async getAllRequests(page: number = 1, limit: number = 10, status?: string, electionId?: number): Promise<PaginatedResponse<Request>> {
  ...
  async getImageBlob(requestId: string, imageType: 'passport' | 'photo'): Promise<string> {
    const token = authService.getAccessToken();
    const response = await axios.get(`${API_BASE_URL}/admin/request/${requestId}/image/${imageType}`, {
      headers: {
